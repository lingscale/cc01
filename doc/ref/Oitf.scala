...
class Oitf(entries: Int = 2)(implicit val p: Parameters) extends Module with CoreParams {
  ...
  io.oitfrd_match_disrs1 := (buf_vld, buf, buf).zipped.map(_ && _.rdwen && _.rdidx === io.dis_rs1idx).reduce(_ && _)
  io.oitfrd_match_disrs2 := (buf_vld, buf, buf).zipped.map(_ && _.rdwen && _.rdidx === io.dis_rs2idx).reduce(_ && _)
  io.oitfrd_match_disrd  := (buf_vld, buf, buf).zipped.map(_ && _.rdwen && _.rdidx === io.dis_rdidx ).reduce(_ && _)
